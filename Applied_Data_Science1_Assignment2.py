...
def world_bank_data(filename, skip):
     ...
     df_years = data.set_index(['Indicator Name','Country Name']).drop(
                                                           ['Country Code',
                                                            'Indicator Code']
                                                            , axis=1)
     df_countries = data.set_index(['Country Name']).drop(['Country Code'
                                                         ,'Indicator Code']
                                                            , axis=1).T
...
df_years = data.set_index(['Indicator Name','Country Name']).drop(
                                                         ...
                                                         'Indicator Code']
                                                            , axis=1)
# Call function
df_data = world_bank_data('API_19_DS2_en_csv_v2_5361599.csv',4)
years = df_data[0]
country = df_data[1]
# Data Prepartion for 1st Bar graph Plot
years_data_EnergyUse = years.loc['Energy use (kg of oil equivalent per capita)'
                                                                ,'2005':'2010']
years_data_EnergyUse = years_data_EnergyUse.dropna(axis=0)
years_data_EnergyUse = years_data_EnergyUse.loc[['Spain',
                                                 'Albania',
                                                 'Cuba',
                                                 'Greece',
                                                 'Croatia',
